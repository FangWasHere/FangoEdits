...
class Patrol:
    ...
    def begin_patrol(self, patrol_cats: List[Cat], patrol_type: str, clan) -> str:
        ...
        # Choose other clan
        all_options = game.clan.all_other_clans + [game.clan]
        all_options.remove(self.clan)
        if game.clan.all_other_clans and len(all_options) > 0:
    ...
    def _add_patrol_cats(self, patrol_cats: List[Cat]) -> None:
        ...
        for cat in patrol_cats:
            ...
            if cat.status.rank == CatRank.MEDICINE_CAT:
                if "healer adult" in self.involved_cats:
                    self.involved_cats["healer adult"].append(cat)
                else:
    ...
    def _get_possible_patrol(
        ...
        if patrol_type != "med" and "healer adult" in self.involved_cats:
            if "normal adult" not in self.involved_cats:
        ...
        patrol_list = get_patrol_list(
            ...
            outsider_rep=will_allow_outsider_patrols(
                small_clan=get_living_clan_cat_count(Cat, self.clan.group_ID)
                < get_config("patrol_generation.small_clan_threshold"),
                clan=self.clan
            ),
            other_clan_rep=game.clan.get_relations(self.other_clan, self.clan, True),
            clan=self.clan,
        )
    ...
    def _patrol_pass_basic_constraints(
        ...
        # CHECK POI
        if not event_for_poi(patrol.poi, self.clan):
            if is_debug_patrol:
